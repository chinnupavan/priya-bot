...
async def home_page():
    embedVar = discord.Embed(title="Command prefix : `p!`\n",
                             description="**Misc - 6 : **\n" + "`id`" + "," + "`avatar`" + "," + "`btc`" + "," + "`ping`" + "," + "`timer <seconds>`" + "," + "`clear <amount>`"+ "," + "`uptime`"
...
async def sample_page():
    embedVar = discord.Embed(title="Misc",
...
async def mining_page():
    embedVar = discord.Embed(title="Mining",
...
class Help(commands.Cog):
    ...
    async def help1(self, ctx: commands.Context):
        ...
        def check(reaction, user):
            return user == ctx.author and str(reaction.emoji) in ["<a:arrright:859808773912395776>",
        ...
        if cur_page == 1:
            message = await ctx.send(content="<a:bluetick:859677933822804019>" + f"**Page : {cur_page}/{pages}: **",
                                     embed=hm_page)
        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == "<a:arrright:859808773912395776>" and cur_page != pages:
                    cur_page += 1
                    if cur_page == 1:
                        await message.edit(content="<a:bluetick:859677933822804019>" + f"Page {cur_page}/{pages}: ",
                                           embed=hm_page)
                    elif cur_page == 2:
                        await message.edit(content="<a:bluetick:859677933822804019>" + f"Page {cur_page}/{pages}: ",
                                           embed=firs_page)
                ...
                elif str(reaction.emoji) == "<a:downleft:859808771122397235>" and cur_page > 1:
                    cur_page -= 1
                    if cur_page == 1:
                        await message.edit(content="<a:bluetick:859677933822804019>" + f"Page {cur_page}/{pages}: ",
                                           embed=hm_page)
