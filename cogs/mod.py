...
class Mod(commands.Cog):
    ...
    async def ban(self, ctx, member: discord.Member, *, reason=None):
        ...
        embed2 = discord.Embed(
            ...
            colour=0xFF0000)
        embed2.add_field(name='Reason ', value=f'{reason}', inline=True)
        embed2.add_field(name='Banned by ',
                         value=f'{ctx.author.name}',
                         inline=True)
        await ctx.send(embed=embed)
        await member.send(embed=embed2)
    ...
    async def unban(self, ctx, member, *, reason=None):
        ...
        for ban_entry in banned_users:
            ...
            if (user.name, user.discriminator) == (member_name,
                ...
                embed.add_field(name='Reason', value=f'{reason}', inline=False)
                embed2 = discord.Embed(
                    description=f'**You were Unbanned from {ctx.guild.name}**',
                    colour=0xFF0000)
                embed2.add_field(name='Reason ',
                                 value=f'{reason}',
                                 inline=True)
                embed2.add_field(name='Unbanned by ',
                                 value=f'{ctx.author.name}',
                                 inline=True)
                await ctx.send(embed=embed)
                await member.send(embed=embed2)
    ...
    async def mute(self, ctx, member: discord.Member, reason=None):
        ...
        embed2 = discord.Embed(
            ...
            colour=0xFF0000)
        embed2.add_field(name='Reason ', value=f'{reason}', inline=True)
        embed2.add_field(name='Muted by ',
                         value=f'{ctx.author.name}',
                         inline=True)
        await ctx.send(embed=embed)
        await member.send(embed=embed2)
    ...
    async def unmute(self, ctx, member: discord.Member, reason=None):
        ...
        embed.add_field(name='Unmuted by ',
                        value=f'{ctx.author.mention}',
                        inline=False)
        embed.add_field(name='Reason', value=f'{reason}', inline=False)
        embed2 = discord.Embed(
            ...
            colour=0xFF0000)
        embed2.add_field(name='Reason ', value=f'{reason}', inline=True)
        embed2.add_field(name='Unmuted by ',
                         value=f'{ctx.author.name}',
                         inline=True)
        await ctx.send(embed=embed)
        await member.send(embed=embed2)
    ...
    async def banlist(self, ctx: commands.Context):
        ...
        if len(ban_list) <= 0:
            ...
            for i in ban_list:
                emb.add_field(name=f"{i.user.name}#{i.user.discriminator}",
    ...
    async def role(self, ctx: commands.Context, mention: discord.Member,
                   role: discord.Role):
