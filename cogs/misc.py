...
class Misc(commands.Cog):
    ...
    async def user(self,
        ...
        em.add_field(name="Name",
                     ...
                     inline=False)
        em.add_field(name="Nickname", value=f"{author.nick}", inline=False)
        em.add_field(name="id", value=f"{author.id}", inline=False)
        em.add_field(name="Account creation",
                     value=f"{author.created_at}",
                     inline=False)
    ...
    async def botinfo(self, ctx: commands.Context):
        ...
        botinfoEmbed.add_field(name="Name : ",
                               value=f"` {bot.name}#{bot.discriminator}    `",
                               inline=False)
        botinfoEmbed.add_field(name="Id : ",
                               value=f"` {botId}  `",
                               inline=False)
        botinfoEmbed.add_field(name="Servers : ",
                               value=f"` {len(servers)}  `",
                               inline=False)
        botinfoEmbed.add_field(name="Members : ",
                               value=f"` {users}  `",
                               inline=False)
        botinfoEmbed.add_field(name="Library : ",
                               value="` discord.py 1.7.3    `",
                               inline=False)
        botinfoEmbed.add_field(name="Created by ",
                               value="` GhostKiller#4676    `",
                               inline=False)

        x = ctx.guild.created_at.now()
        botinfoEmbed.add_field(
            name="Created At",
            value=
            f"`{x.year}-{x.month}-{x.day}` `{x.hour}hrs {x.minute}min {x.second}sec`",
            inline=False)
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        emb.add_field(name="Emojis",
                      value=f"`{len(guild.emojis)}`",
                      inline=True)
        emb.add_field(name="Verification level",
    ...
    async def logs(self, ctx: commands.Context, amt=None):
        ...
        async for entry in ctx.guild.audit_logs(limit=a):
            ...
            embed.add_field(
                name=c,
                value='`{0.user} did {0.action} to {0.target}`'.format(entry),
                inline=False)
            c += 1
