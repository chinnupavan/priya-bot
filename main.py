...
async def change_pr():
    ...
    for g in client.guilds:
        for i in g.members:
            c = c + 1
    await client.change_presence(activity=discord.Activity(
        type=discord.ActivityType.playing, name=f"p!help | {c} Users"),
                                 status=discord.Status.idle)
...
async def on_member_join(member):
    ...
    try:
        ...
        embed = discord.Embed(title="Welcome to " + member.guild.name,
                              description=member.mention + " is our " +
                              str(len(member.guild.members)) + "th member.")
        file = discord.File("b3.png")
        embed.set_image(url="attachment://b3.png")
        db = sqlite3.connect('server.db')
        cursor = db.cursor()
        cursor.execute(
            f"SELECT * FROM main WHERE server_id = {member.guild.id}")
        result = cursor.fetchone()
        db.commit()
        cursor.close()
        db.close()
        ...
        embed.set_footer(text="Totale members : " +
...
async def on_member_remove(member):
    ...
    try:
        ...
        em = discord.Embed(title="Bye bye " + member.name + "!",
        ...
        if 'p!nsfw' in message.content.lower():
            if message.channel.is_nsfw():
                ...
                await message.channel.send(
                    "Please use this command in NSFW channel" +
...
async def blur(ctx):
    ...
    embed = discord.Embed(title="Welcome to " + ctx.guild.name,
    ...
    try:
        async with aiohttp.ClientSession() as session:  # Async HTTP request
            ...
            embedd = discord.Embed(
                description="<a:btcinr:860587207398916096>" +
